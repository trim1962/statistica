...
Per parlare di statistica partiamo da un esempio pratico. Supponiamo di avere una classe di venti alunni. La~\vref{tab:alunnovoto1} riporta i risultati di una verifica. L'insieme dei dati costituisce una popolazione\index{Popolazione}. Una popolazione è formata da unità statistiche\index{Unità statistiche}, in questo caso i voti ma potrebbero essere misure, il numero di punti fatti da un giocatore di basket.   

Le informazioni che fornisce questa tabella non vanno oltre il semplice elenco dei risultasti. Non otteniamo un risultato migliore  considerando la~\vref{fig:VotiAlunni} associato alla tabella. In ordinata abbiamo i voti in ascissa i nomi degli alunni. Ma come è andata la prova per la classe? Per dare una prima risposta  definiamo il concetto di frequenza. Si dice che un dato ha una frequenza se  si ripete più volte. 
 \begin{table}
	\centering
...
La~\vref{tab:FrequenzeConfronto} riepiloga quanto detto in precedenza confrontando fra loro i vari risultati. Per ottenelre tale formula si può utilizzare un foglio elettronico come quello~\vpageref[foglio]{tab:FrequenzeaConfrontoExcel}
\begin{table}[!t]
	\centering
