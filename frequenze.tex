...
Ciò  che  ci dice questa tabella non va oltre il semplice elenco dei risultasti. Non otteniamo un risultato migliore  considerando la~\vref{fig:VotiAlunni} associato alla tabella. In ordinata abbiamo i voti in ascissa i nomi degli alunni. Ma come è andata la prova per la classe? Per dare una prima risposta  definiamo il concetto di frequenza.
\begin{defn}[Frequenza]
	Si dice che un dato ha una frequenza\index{Frequenza} se  si ripete più volte.
\end{defn}
 \begin{table}
	\centering
...
Problema come conoscere il numero dei ragazzi che hanno ottenuto una valutazione inferiore ad un dato valore? Per rispondere a questa domanda dobbiamo introdurre un nuovo concetto, quello di frequenza comulata.
\begin{defn}[Frequenza cumulata]
	La frequenza cumulata\index{Frequenza!cumulata} di una data unità è pari alla sua frequenza assoluta più la somma di tutte quelle che la  precedono.
\end{defn}
